...
public class UserAccountService {
	...
	public void registerUser() {
		...
		try {
			if (request.getParameter("user") .equals("")
					...
					|| request.getParameter("email").equals("") || request.getParameter("phone").equals("") || request.getParameter("address").equals("") || request.getParameter("zipcode").equals("")) {
				vsl_Context.put("username", request.getParameter("user"));
				vsl_Context.put("company", request.getParameter("company"));
				...
				/**
				 * Checking if twofactor was checked, giving the write number
				 * for database
				 */
				if (request.getParameter("twofactor") != null) {
					postParameters
							.add(new BasicNameValuePair("twofactor", "1"));
				} else {
					postParameters
							.add(new BasicNameValuePair("twofactor", "0"));
				}
			...
			if (json != null) {
				...
				if (!json.containsKey("error")) {
					...
					if (isValid && !isAndroid) {
						...
						// message if checked
						setLoginUserSession(request, response, json);

					}

					/**
					 * If the request came from Android, the user wants to
					 * register for the two factor authentication, the method
					 * will be called
					 */
					else if (isAndroid) {
	 ...
	 * @throws IOException
	 */
	private void setLoginUserSession(HttpServletRequest request,
	...
	public void updateUserAccountDetails(long sessionUserID) {
		...
		/**
		 * Finally statement for saving the user input when an error
		 * code appeared
		 */
		finally{
			vsl_Context.put("userDetails", custBean);
